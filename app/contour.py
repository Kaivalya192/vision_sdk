...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _publish_vgr_result(self, dets: List[Dict[str, Any]]):
        """
        Emit VGR VisionResult JSON (vision_bridge → /vgr/vision_result) in the
        'vision_ui' format, including 'pose' and 'template_size' so the manager
        uses the calibrated mapping (prevents IK blowups).
        """
        if not self.chk_pub_robot.isChecked():
            return

        # Name → bucket mapping (case-insensitive) to hit your Sorting map
        def bucketize(n: str) -> str:
            k = (n or "").strip().lower()
            if k in ("obj1", "red", "obj3", "go"):
                return "go"
            if k in ("obj2", "yellow", "obj4", "nogo"):
                return "nogo"
            return "nogo"
        ...
        if require_active and not tpl_selected:
            payload = {
                "version":"1.0",
                "sdk":"vision_ui",   # important: match the working sender id
            ...
            self._append_log("VGR ← empty (require active template ON)")
            return

        best_det = None
        send_name = "nogo"
        score = 1.0  # keep positive

        if tpl_selected:
            tpl_name = self._templates[self._active_index]["name"]
            candidates = [d for d in dets if d.get("match") is not None and np.isfinite(d["match"]) and d["match"] <= thr]
            if candidates:
                best_det = min(candidates, key=lambda d: d["match"])
                send_name = bucketize(tpl_name)
                m = float(best_det.get("match", 0.0))
                score = 1.0 / (1.0 + max(0.0, m))
        ...
        if best_det is None:
            if not dets:
                ...
                return
            # No passing template → send largest as 'noAgo'
            best_det = dets[0]
            send_name = "nogo"
            score = 1.0

        # Build detection in 'vision_ui' shape
        ...
        det_obj = {
            ...
            "inliers": inliers,
            "pose": {
                # pixel pose (manager applies calibration / swap_xy itself)
                "x": float(cx),
                "y": float(cy),
                "theta_deg": float(theta),
                "x_scale": 1.0,
                "y_scale": 1.0,
                # seed origin for quad; first corner is fine
        ...
        obj = {
            "object_id": 1,
            "name": send_name,            # 'go' / 'nogo' so Sorting map hits
        ...
        self.pub_vgr.send_json(payload)
        self._append_log(f"VGR ← {send_name}  score={score:.3f} px=({cx:.1f},{cy:.1f}) yaw={theta:.2f}°  size={size_wh}")
